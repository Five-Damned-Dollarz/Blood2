...
const char* GetImpactSoundDirectory(const SurfaceType eSurfType) // 210307 - added to help fix memory write access violation (char* vs. const char*)
{
	switch (eSurfType)
	{
	case SURFTYPE_STONE:
...
char* CWeaponFX::GetImpactSound(DBYTE nWeaponType, DBYTE nAmmoType, SurfaceType eSurfType) // 210307 - rewritten to avoid memory access violation
{
	int nMax = NRES(4) - 1;
	int random = GetRandom(0,nMax);

	if (const char* impact_dir = GetImpactSoundDirectory(eSurfType))
	{
		sprintf(s_FileBuffer, "Sounds\\Weapons\\impacts\\%s\\impact%d.wav", impact_dir, '1' + random);
		return s_FileBuffer;
	}
