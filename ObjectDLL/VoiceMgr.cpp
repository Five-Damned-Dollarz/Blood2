...
int CVoiceGroup::AddSoundStrings(char* sDir)
{
	...
	while (pCurFile)
	{
		if (pCurFile->m_Type == TYPE_FILE)
		{
			...
			if (nLen > 4 && _strnicmp(&pCurFile->m_pBaseFilename[nLen - 4], ".wav", 4) == 0)
			{
...
DBOOL CVoiceMgr::PlayEventSound(CPlayerObj* pPlayerObj, int iEvent)
{
	// Sanity checks...

	if (!pPlayerObj) return(DFALSE);
	if (!IsInited()) return(DFALSE);
	...
	int iCharacter = pPlayerObj->GetCharacter();
	...
	{
		if (!PlaySoundNoStream(pPlayerObj->m_hObject, sSound))
		{
			return(DFALSE);
		}
	}
	...
	{
		if (!PlaySound(pPlayerObj->m_hObject, sSound))
		{
			return(DFALSE);
		}
	}
...
DBOOL CVoiceMgr::PlayUniqueSound(CPlayerObj* pPlayerObj, int iUnique)
{
	// Sanity checks...
	...
	// Unique sounds are only for caleb...
...
DBOOL CVoiceMgr::IsOkToPlaySound()
{
	...
	if (m_hCurSound)
	{
		...
		if (m_pServerDE->IsSoundDone(m_hCurSound, &bDone) == DE_OK)
		{
			if (!bDone) return(DFALSE);
		}
	}

	return(m_pServerDE->GetTime() > m_fNextPlayTime);
}
...
DBOOL CVoiceMgr::PlaySound(HOBJECT hObject, char* sSound, DFLOAT fTimeAdd)
{
	// Sanity checks...
	...
	m_hCurSound = PlaySoundFromObject(hObject, sSound, 1400, SOUNDPRIORITY_PLAYER_MEDIUM, DFALSE, DTRUE, DTRUE, 100, DTRUE, DTRUE);
	...
	DFLOAT  fSoundTime = 1;
	DRESULT dr         = m_pServerDE->GetSoundDuration(m_hCurSound, &fSoundTime);
...
DBOOL CVoiceMgr::PlaySoundNoStream(HOBJECT hObject, char* sSound, DFLOAT fTimeAdd)
{
	// Sanity checks...
	...
	m_hCurSound = PlaySoundFromObject(hObject, sSound, 1400, SOUNDPRIORITY_PLAYER_MEDIUM, DFALSE, DTRUE, DTRUE, 100, DTRUE, DFALSE);
	if (!m_hCurSound) return(DFALSE);
